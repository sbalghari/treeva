...
from yada.utils.exceptions import ThemeConfigError, ConfigNotFound


_PLACEHOLDER_RE = re.compile(r"\{([a-zA-Z_][\w]*)\.([^\|}]+)(?:\|([^\}]+))?\}")


class _ConfigResolvedDict(dict):
    """
...
def read_config(path: Path) -> Dict:
    ...
    return _ConfigResolvedDict(data)
...
def read_rich_theme(theme_path: Path) -> Dict:
    ...
    """
    return _OneLevelFlatDict(read_config(theme_path))
