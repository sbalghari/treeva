...
Public API for the ``treeva agents`` command: scan the project once,
render the requested sections, and merge them into AGENTS.md files
with per-section markers so each section can be generated, updated,
...
@dataclass
class AgentsWrite:
    """A single AGENTS.md file to write.

    Attributes:
        path: Absolute target path.
        content: Full merged content to write.
...
class GenerateResult:
    ...
    writes: list[AgentsWrite] = field(default_factory=list)
    ...
    *,
    ...
    for rel, blocks in _requested_files(all_blocks, names).items():
        ...
        if merged is not None:
            result.writes.append(AgentsWrite(path=target, content=merged))
